...
def get_main_filing_document(index_url):
    ...
    if not table:
        print("⚠️ Could not find document table.")
        return None
    ...
    for row in rows:
        ...
        if len(cols) >= 4:
            ...
            if "10-k" in doc_desc or "complete submission text file" in doc_desc:
                ...
                print(f"📄 Found filing document: {doc_name} – {full_link}")
                return full_link

    print("⚠️ No main document (10-K or full text) found.")
    return None
...
if doc_url:
    ...
    print("❌ No filing document extracted.")
...
def extract_filing_text(document_url):
    ...
    print(f"📄 Downloading filing: {document_url}")
    response = requests.get(document_url, headers=headers)

    if response.status_code != 200:
        print(f"❌ Failed to retrieve document. Status: {response.status_code}")
        return ""
...
def save_topics_to_drive(topic_model, chunks, topics, filename="walmart_10k_topics.json"):
    ...
    print(f"✅ Saved {len(output)} topic chunks to {full_path}")
...
def process_company_filings(
    ...
    for i, index_url in enumerate(index_urls):
        try:
            ...
            if not main_url:
                print(f"⚠️ No filing document found for {index_url}")
                continue
            ...
            if len(chunks) < 2:
                print(f"⚠️ Skipping {company_name} filing #{i+1}: Not enough chunks for BERTopic.")
                continue
            ...
            with open(save_path, "w", encoding="utf-8") as f:
                json.dump(output, f, indent=2)
            print(f"✅ Saved {len(output)} topics to {save_path}")
        except Exception as e:
            print(f"❌ Failed to process filing #{i+1} for {company_name}: {e}")
...
def batch_process_companies(company_cik_map, filings_to_fetch=2, form_type="10-K"):
    ...
    for name, cik in company_cik_map.items():
        try:
            ...
            all_status.append({"Company": name, "Status": "✅ Success"})
        except Exception as e:
            print(f"❌ Error with {name}: {e}")
            all_status.append({"Company": name, "Status": f"❌ Failed: {str(e)}"})
    return all_status
company_cik_map = {
